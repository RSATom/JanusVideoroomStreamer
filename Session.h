...
private:
    ...
    void sendFeatures();
    bool handleFeaturesReply(const JsonPtr&);

    void sendWatch();
    bool handleWatchReply(const JsonPtr&);

    void sendTrickle(unsigned mlineIndex, const std::string& candidate);
    bool handleTrickleReply(const JsonPtr&);

    void sendStart(const std::string& sdp);
    bool handleStartReply(const JsonPtr&);
...
private:
    ...
    std::unique_ptr<WebRTCPeer> _viewPeerPtr;
};

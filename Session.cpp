...
char const * const Plugin = "janus.plugin.streaming";
...
void Session::sendWatch()
{
    ...
    json_object_set_new(jsonBody, "id", json_integer(1));
...
bool Session::handleWatchReply(const JsonPtr& jsonMessagePtr)
{
    ...
    assert(ExtractString(pluginDataJson, "plugin") == Plugin);

    json_t* dataJson = json_object_get(pluginDataJson, "data");
    if(!dataJson)
        return false;

    assert(ExtractString(dataJson, "streaming") == "event");
    ...
    assert(ExtractString(resultJson, "status") == "preparing");
...
{
    spdlog::info("Answer SDP: {}", _viewPeerPtr->sdp());
